...
class APISite(object):
	...
	def register(self, model_or_iterable, api_class = None, **options):
		...
		for model in model_or_iterable:
			...
			self.logger.debug('Registered API for %s' % unicode(model._meta.verbose_name))
	...
	def unregister(self, model):
		...
		self.logger.debug('Unregistered API for %s' % unicode(model._meta.verbose_name))
	...
	def api_view(self, view, request, format, *args, **kwargs):
		...
		try:
			...
			if settings.DEBUG:
				...
				return APIResponse(format, request, ex, detail_level = detail_level)
		
		return APIResponse(format, request, data, detail_level = detail_level)
