...
class NavigationSite(object):
	...
	def register(self, builder):
		...
		self.logger.info('Registered menu builder %s' % builder)
	...
	def build(self, name, auth = NAVIGATION_SHOW_ALL, logged_in = False, *partials):
		...
		for partial in plist:
			items.extend(
				self.build_partial(partial, auth, logged_in)
			)
		
		return sorted(items, key = lambda k: k.get('order', 0))
	
	def build_partial(self, name, auth = NAVIGATION_SHOW_ALL, logged_in = False):
		items = []
		exclude = []
		instances = []
		...
		for builder in instances:
			builder.add_to_menu(name, ilist)
